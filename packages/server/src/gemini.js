function typeHint(type, key, label) {
    if (type === "duration")
        return `duration string like "2h21" or "1h30m" or "45m"`;
    if (type === "number") {
        const l = label.toLowerCase();
        if (l.includes("imdb"))
            return "decimal 0.0–10.0";
        if (l.includes("rotten") || l.includes(" rt"))
...
export async function extractFromImage(imageBase64, mimeType, scope, apiKey, model = "gemini-2.0-flash-lite") {
