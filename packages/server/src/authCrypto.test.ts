...
// Same RFC 7638 §3.1 worked example as the client's clientIdentity.test.ts —
// kept identical on both sides so a fixed external vector, not just mutual
// agreement, is what's being checked.
...
describe("canonicalJwkString (server side)", () => {
  it("matches the RFC 7638 §3.1 RSA example", () => {
...
describe("verifyAuthSignature", () => {
  ...
  it("rejects the same signature replayed against a different server_id — the §3.3 cross-server replay case", async () => {
    const { jwk, privateKey } = await generateP256();
    const clientId = await jwkThumbprint(jwk);
    // Signed for server A...
    const sig = await sign(privateKey, "server-A", "nonce-1", clientId);
    // ...must not verify for server B, even with the identical nonce and
    // client_id — this is the property that makes it safe to bind server_id
    // into the payload rather than relying solely on nonce uniqueness. A
    // true black-box WS reproduction would need two independent server
    // processes to issue the exact same 128-bit random nonce, which isn't
    // practically constructible, so this is tested directly at the
    // signature layer instead — see index.test.ts for the WS-level
    // approximation (a foreign/unrecognized nonce is rejected).
