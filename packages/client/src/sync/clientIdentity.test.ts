...
// The RFC 7638 §3.1 worked example — the same RSA key used across several
// JOSE RFCs (7515/7517/7638). Included so the shared canonicalize+hash+
// base64url pipeline is checked against a value this module didn't produce
// itself, even though production only ever exercises the "EC" branch.
...
describe("canonicalJwkString", () => {
  it("keeps only the required members, sorted, with no whitespace (RFC 7638 §3.1 RSA example)", () => {
    expect(canonicalJwkString(RFC7638_RSA_JWK)).toBe(
      `{"e":"AQAB","kty":"RSA","n":"${RFC7638_RSA_JWK.n}"}`,
    );
  });

  it("produces the exact EC canonical form the design doc specifies (§4.1)", () => {
...
describe("base64UrlFromBytes", () => {
  it("is unpadded and URL-safe (no +, /, or =)", () => {
    // 0xff 0xff 0xff base64s to "////" with standard alphabet — a good probe
    // for the -/_ substitution actually firing.
...
describe("buildAuthPayload", () => {
  ...
  it("differs when server_id changes, holding nonce and client_id fixed", () => {
    // This is the property that stops a signature captured by one server
    // from being replayed against another (§4.2, §3.3): changing server_id
    // alone must change every byte from that point on, so a signature valid
    // for server A can never verify for server B even if by some
    // vanishingly unlikely coincidence both issued the exact same nonce to
    // the exact same client_id. (A true black-box WS test of this exact
    // scenario would require forcing a 128-bit nonce collision across two
    // independent server processes, which isn't practically constructible —
    // see index.test.ts's "nonce bound to a different server_id" test for
    // the WS-level approximation and a note on this gap.)
